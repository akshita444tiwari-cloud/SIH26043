...
import { ArrowLeft, ArrowRight, Building2, GraduationCap, Info, Landmark, Users, UserSquare2 } from 'lucide-react'
import { JharkhandEmblem, CornerMandala, FolkBorder } from '@/components/cultural/motifs'
import type { Role } from '@/lib/types'

export const metadata = {
  title: 'Login',
  description: 'Choose a role to explore the Jharkhand societal innovation platform prototype.',
}

const roles: { role: Role; icon: typeof Users; title: string; desc: string; tone: string }[] = [
  {
    role: 'citizen',
    icon: Users,
    title: 'Citizen',
      ...
      <div className="relative mx-auto flex min-h-screen max-w-5xl flex-col px-4 py-8 md:px-6">
        ...
        <div className="mt-8 flex flex-col items-center text-center">
          ...
          <p className="mt-3 max-w-xl text-foreground/70">
            Every stakeholder gets a purpose-built workspace. Select a role below to explore its dashboard.
          </p>
        </div>
        ...
        <div className="mt-10 grid flex-1 content-start gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {roles.map((r) => (
            ...
            >
              <span className={`grid size-12 place-items-center rounded-2xl ${r.tone}`}>
                <r.icon className="size-6 text-forest-deep" />
              </span>
              <h2 className="mt-4 font-serif text-lg font-semibold text-forest-deep">{r.title}</h2>
              <p className="mt-1 flex-1 text-sm text-foreground/70">{r.desc}</p>
        ...
        <div className="mt-8 flex items-start gap-2 rounded-xl border border-border bg-muted/50 p-4 text-sm text-muted-foreground">
          ...
          <p>
            Select your role to enter its dedicated workspace. Each stakeholder sees the tools, data, and actions
            relevant to their part of the collaborative problem-solving journey.
          </p>
        </div>
      </div>
