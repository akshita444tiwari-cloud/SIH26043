      ...
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3 md:px-6">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="font-serif text-base font-bold text-forest-deep md:text-lg">Government of Jharkhand</div>
          </div>
        </Link>
          ...
          {NAV.map((n) => (
            <Link
              ...
              className={cn(
                ...
                isActive(n.href) ? 'text-secondary' : 'text-foreground/70 hover:text-forest-deep',
              )}
            >
              {n.label}
              {isActive(n.href) && <span className="absolute inset-x-3 -bottom-px h-0.5 rounded-full bg-secondary" />}
            </Link>
          ))}
        </nav>
          ...
          {NAV.map((n) => (
            <Link
              ...
              className={cn(
                ...
                isActive(n.href) ? 'bg-primary/10 text-secondary' : 'text-foreground/75',
              )}
            >
              {n.label}
            </Link>
